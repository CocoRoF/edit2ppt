...
from edit2ppt.config import get_settings, reset_settings_cache
...
async def standalone_env(tmp_path, monkeypatch):
    ...
    Sets EDIT2PPT_DATA_DIR + clears DATABASE_URL/REDIS/S3 so we exercise
    the standalone defaults end-to-end.
    """
    monkeypatch.setenv("EDIT2PPT_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.delenv("EDIT2PPT_DATABASE_URL", raising=False)
    monkeypatch.delenv("EDIT2PPT_REDIS_URL", raising=False)
    monkeypatch.delenv("EDIT2PPT_S3_ENDPOINT_URL", raising=False)
    monkeypatch.delenv("EDIT2PPT_S3_BUCKET", raising=False)
    monkeypatch.setenv("EDIT2PPT_RAW_SIGNING_KEY", "test-signing-key" * 4)
    monkeypatch.delenv("EDIT2PPT_AUTH_DEV_API_KEY", raising=False)
    ...
    from edit2ppt.db import session as session_mod
    from edit2ppt.storage import set_default_storage

    session_mod.reset_engine_cache()
    set_default_storage(None)
    # Drop any cached default storage instance from a prior test run.
    from edit2ppt.storage import get_default_storage
...
async def app(standalone_env):
    ...
    # Importing late is critical — `from edit2ppt.api.main import app` reads
    ...
    from edit2ppt.api.main import app  # noqa: PLC0415
...
class TestBootstrap:
    ...
    async def test_sqlite_file_created_with_tables(self, client, standalone_env):
        sqlite_path = standalone_env.data_dir / "edit2ppt.db"
        assert sqlite_path.exists()

        # All six tables present.
        from edit2ppt.db.session import get_engine
