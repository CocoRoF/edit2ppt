...
Run with:
    .venv/bin/arq edit2ppt.workers.main.WorkerSettings

Or with reload during dev:
    .venv/bin/arq edit2ppt.workers.main.WorkerSettings --watch src/
...
async def startup(ctx: dict) -> None:
    ...
    logger.info("edit2ppt worker starting", extra={"env": settings.environment})
    ctx["sessionmaker"] = get_sessionmaker()
    ctx["bus"] = get_default_bus()


async def shutdown(ctx: dict) -> None:
    logger.info("edit2ppt worker shutting down")
