...
def generate_image(req: GenerateImageRequest) -> GenerateImageResponse:
    ...
    module_name = f"edit2ppt.core.image_backends.backend_{req.backend}"
    try:
    ...
    with temp_workspace(prefix=f"edit2ppt-img-gen-{req.backend}-") as ws, _env_overlay(req.api_keys):
...
def search_image(req: SearchImageRequest) -> SearchImageResponse:
    ...
    with temp_workspace(prefix="edit2ppt-img-search-") as ws, _env_overlay(req.api_keys):
